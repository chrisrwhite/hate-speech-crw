...
if dataset == 'hate_nostop':
    extract_df = pd.read_sql_query("SELECT extract from hate_universal_encoder_embedding_features",con)
    ...
    extract_df = pd.read_sql_query("SELECT extract from toxic_universal_encoder_embedding_features", con)
...
# reset indexes for
df3 = extract_df.reset_index(drop = False)
...
df_4.to_csv("word_embedding_df.txt", sep = ' ', header = False, index=True)
df_4.head(10)
